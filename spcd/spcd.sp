...
{   spcd fetch           read deps.sp, clone deps, write spcd_lib/lock.sp       }
...
[
  ...
  [
    `spcd: writing spcd_lib/lock.sp` log/info
    tmp-new fmap/sort-u
    `mkdir -p spcd_lib` sh/! drop
    `(echo '{ generated by spcd -- do not edit by hand }' && cat ` tmp-new str/cat
    `) > spcd_lib/lock.sp` str/cat sh/! drop
...
[
  ...
  [
    spcd-set-lock
    `cat spcd_lib/lock.sp` sh/> eval
  ]
  `test -f spcd_lib/lock.sp` sh/! 0 =
  if
...
[
  ...
  [ `grep -v '^{' spcd_lib/lock.sp | grep -v '^$' || true` sh/> . ]
  `test -f spcd_lib/lock.sp` sh/! 0 =
  if
  spcd-cleanup
] [verb-list] @
